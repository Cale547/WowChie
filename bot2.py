...
MY_GUILD = discord.Object(id=935927889373847552)  # replace with your guild id
...
@app_commands.describe(
    aoc_name="Your username on Advent of Code, make sure to remove spaces"
)
async def register(interaction: discord.interactions, aoc_name: str):
    """Connects your discord name to your AoC name"""
    print(f"Tries to register {interaction.user.name} to {aoc_name}")
    with open("userbase.txt", 'r') as f:
        content = f.read()
        if re.search(f".*{interaction.user.name}.*", string=content) or re.search(aoc_name, string=content):
            await interaction.response.send_message("It looks like your discord name or AoC name is already registered, contact system administrator (Kalle)")
            return -1
    with open("userbase.txt", 'a') as f:
        f.write(f"{aoc_name},{interaction.user.name}\n")
...
async def senast(interaction: discord.interactions):
    print("Showing the most recent star-getter")
    latest_time = 0
    with json.loads("storedScores.json") as f:
